...
class FrameProcessor:
    ...
    # def extract_roi(self, frame, buffer):
    ...
    def extract_roi(self, frame, buffer = 10):
        ...
        # Tính bounding box polygon gốc
        x, y, w, h = cv2.boundingRect(self.roi)

        # Mở rộng bounding box theo buffer, giới hạn trong ảnh
        x1 = max(x - buffer, 0)
        y1 = max(y - buffer, 0)
        x2 = min(x + w + buffer, frame.shape[1])
        y2 = min(y + h + buffer, frame.shape[0])

        # Tạo mask mới hình chữ nhật vùng mở rộng
        cv2.rectangle(mask, (x1, y1), (x2, y2), 255, thickness=-1)

        # Lấy ROI mở rộng nhưng giữ nguyên kích thước ảnh gốc
        roi = cv2.bitwise_and(frame, frame, mask=mask)

        # Vẽ overlay polygon gốc lên frame
        overlay = frame.copy()
        color = (255, 255, 150)
        alpha = 0.1
        ...
        # (Tuỳ chọn) vẽ bounding box buffer (màu đỏ)
